...
const endpointTrend = {
  ...
  test: new Trend("test_duration"),

  get_branch_lof: new Trend("get_branch_lof_duration"),
  get_all_lof: new Trend("get_all_lof_duration"),
  get_all_log: new Trend("get_all_log_duration"),

  organizations: new Trend("organizations_duration"),
  cron_organizations: new Trend("cron_organizations_duration"),
  organization_cycle: new Trend("organization_cycle_duration"),
  get_cron: new Trend("get_cron_duration"),

  // Cards
  get_cards: new Trend("get_cards_duration"),
  register_card: new Trend("register_card_duration"),
  confirm_card: new Trend("confirm_card_duration"),

  // Payments
  payment_one: new Trend("payment_one_duration"),
  payment_reverse: new Trend("payment_reverse_duration"),

  // PWR
  pwr_sms: new Trend("pwr_sms_duration"),
  pwr_confirm: new Trend("pwr_confirm_duration"),
};
...
export default function () {
  ...
  if (random < 0.1) {
    ...
    response = http.get(`${config.baseUrl}/health`, {
      tags: { endpoint },
    });
  } else if (random < 0.2) {
    endpoint = "test";

    response = http.get(`${config.baseUrl}/test`, {
      headers: authHeaders(),
      tags: { endpoint },
    });
  } else if (random < 0.45) {
    ...
    response = http.get(`${config.baseUrl}/lofs/get-all-lof?page=1`, {
      headers: authHeaders(),
      tags: { endpoint },
    });
  } else if (random < 0.72) {
    ...
    response = http.get(`${config.baseUrl}/logs/get-all-log`, {
      headers: authHeaders(),
      tags: { endpoint },
    });
  } else if (random < 0.8) {
  ...
  if (endpointTrend[endpoint]) {
    endpointTrend[endpoint].add(response.timings.duration);
  }
